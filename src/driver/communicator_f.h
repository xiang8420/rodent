...
//        printf("dst %d src %d \n", dst, src);
    ...
    bool read_from_file(struct Ray_Queue &queue, int capacity){
        DIR *dp;
        struct dirent *dirp;
    ...
    int file_rays_transfer(float *primary, size_t size, size_t capacity) {
		...
		while( primary_size < capacity && !first.empty()) {			
            ...
            if(first.empty()) { read_from_file(first, capacity); }
		}
...
//                    printf("\n\nprimary %d  %d %d\n\n", primary_size, i, num_rays[i]);
            ...
            while(1){
                usleep(300);
                if(any_working()){   
                    if(read_from_file(first, capacity))
                        break;
                } else {
                    if(read_from_file(first, capacity))
                        break;
